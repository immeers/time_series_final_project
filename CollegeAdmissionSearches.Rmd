...
for (i in (1:length(file_paths))){
  ...
  ma.trailing.pred <- ts(rep(last.ma.trailing, n_valid[[i]]), start = c(2020, nTrain + 1), 
                       end = c(2020, nTrain + nValid), freq = 52)
...
#### Holts Winters - IM
Holts Winters is not able to handle a frequency higher than 24. The dataset for analysis has a frequency of 52 (weekly).
```{r, eval = FALSE}
...
#### ETS
For a frequency higher than 24, it is recommended to use STLF, which implicity uses an ETS model.
```{r}
ets_models = list()
...
Using MAPE
```{r}
model_names <- c("SNAIVE", "SES", "TSLM", "ARIMA", "MA TRAILING", "MA CENTERED", "NNAR", "ETS")
...
for (i in (1:length(file_paths))){
  ...
  ma_centered_mape <- accuracy(ma_centered, valid_data[[i]])["MAPE"]
  nnar_mape <- accuracy(nnar, valid_data[[i]])["Test set", "MAPE"]
  ets_mape <- accuracy(ets, valid_data[[i]])["Test set", "MAPE"]
  
  mape_values <- c(snaive_mape, ses_mape, tslm_mape, arima_mape, ma_trailing_mape, ma_centered_mape, nnar_mape, ets_mape)
...
A forecast for the next year will be created for each school using their respective best model from above.
  
Harvard: ETS
```{r, message = False}
harvard.full <- stlf(data_sets[[1]])
harvard.forecast <- forecast(harvard.full, h = 52)
autoplot(harvard.forecast) + ggtitle("Harvard Forecast")
...
ggplot(differences, aes(x = Date, y = Change)) + 
  geom_line() +
  ggtitle("2025/26 Application Period Forecast - Harvard") +
  geom_vline(xintercept = as.Date(c('2024-11-01', '2025-01-01')), 
             linetype = "dashed", color = "red", size = 1) +
  labs(x = "Date", y = "Change in Admissions") +
  theme_minimal() +
...
autoplot(mit.forecast) + ggtitle("MIT Forecast")
...
ggplot(differences, aes(x = Date, y = Change)) + 
  geom_line() +
  ggtitle("2025/26 Application Period Forecast - MIT") +
  geom_vline(xintercept = as.Date(c('2024-11-01', '2025-01-01')), 
             linetype = "dashed", color = "red", size = 1) +
  labs(x = "Date", y = "Change in Admissions") +
  theme_minimal() +
...
Notre Dame: SNAIVE
```{r, message = False}
nd.full <- snaive(data_sets[[3]])
nd.forecast <- forecast(nd.full, h = 52)
autoplot(nd.forecast) + ggtitle("Notre Dame Forecast")
...
ggplot(differences, aes(x = Date, y = Change)) + 
  geom_line() +
  ggtitle("2025/26 Application Period Forecast - Notre Dame") +
  geom_vline(xintercept = as.Date(c('2024-11-01', '2025-01-01')), 
             linetype = "dashed", color = "red", size = 1) +
  labs(x = "Date", y = "Change in Admissions") +
  theme_minimal() +
...
Princeton: ETS
```{r, message = False}
princeton.full <- stlf(data_sets[[4]])
princeton.forecast <- forecast(princeton.full, h = 52)
autoplot(princeton.forecast) + ggtitle("Princeton Forecast")
...
ggplot(differences, aes(x = Date, y = Change)) + 
  geom_line() +
  ggtitle("2025/26 Application Period Forecast - Princeton") +
  geom_vline(xintercept = as.Date(c('2024-11-01', '2025-01-01')), 
             linetype = "dashed", color = "red", size = 1) +
  labs(x = "Date", y = "Change in Admissions") +
  theme_minimal() +
...
autoplot(stanford.forecast) + ggtitle("Stanford Forecast")
...
ggplot(differences, aes(x = Date, y = Change)) + 
  geom_line() +
  ggtitle("2025/26 Application Period Forecast - Stanford") +
  geom_vline(xintercept = as.Date(c('2024-11-01', '2025-01-01')), 
             linetype = "dashed", color = "red", size = 1) +
  labs(x = "Date", y = "Change in Admissions") +
  theme_minimal() +
...
autoplot(yale.forecast) + ggtitle("Yale Forecast")
...
ggplot(differences, aes(x = Date, y = Change)) + 
  geom_line() +
  ggtitle("2025/26 Application Period Forecast - Yale") +
  geom_vline(xintercept = as.Date(c('2024-11-01', '2025-01-01')), 
             linetype = "dashed", color = "red", size = 1) +
  labs(x = "Date", y = "Change in Admissions") +
  theme_minimal() +
