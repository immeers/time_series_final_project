...
```{r}
random_walk_test <- data.frame("AR1_Coeff" = ar, "S.E" = se, "T_Stat" = t_stat, "Significant" = -1.96 > t_stat)
random_walk_test
```
...
best_plots[[3]] = autoplot(nd.forecast) + ggtitle("Notre Dame Forecast") + ylab("Search Interest")
...
For the following schools we would expect to have a positive change in number of searches for admissions and thus increase popularity, competitiveness and ranking.

  - Stanford
  - MIT
  
For the following schools we would expect to have a negative change in number of searches for admissions and thus decrease popularity, competitiveness and ranking. 
...
## Recommendations and Further Application

We would recommend that high-achieving high schoolers apply to one of the "positive" schools and one of the "negative" schools if they are looking to get into these top institutions. This way you are covering your basis, with a reach and a target school (relatively!) A further application would be to forecast to the date that you are likely to graduate, as this is when ranking will be most important, and choose a school that would be less competitive at admission but more competitive at graduation.
